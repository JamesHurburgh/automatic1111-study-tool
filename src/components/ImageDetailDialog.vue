<template>
  <v-dialog fullscreen v-model="showImageDetailDialog">
    <v-card>
        ...
        <v-container fluid class="primary fill-height pa-0">
          <v-row fluid class="fill-height ma-0 pa-0" no-gutters>
            <v-col cols="8" no-gutters>
              ...
              <v-img :src="image.id" :alt="image.metaData?.prompt"
              :height="$vuetify.display.height"></v-img>
            </v-col>
            <v-col cols="4">
              ...
              <v-textarea readonly density="compact" label="Prompt" hide-details="auto"
                :model-value="image.metaData?.prompt" append-outer-icon="mdi-content-copy"
                @click:append-outer="copy(image.metaData?.prompt)"></v-textarea>
